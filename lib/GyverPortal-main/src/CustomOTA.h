...
public:
    ...
    */
    void defBuild(bool UpdateEnd, const String& UpdateError) {
        ...
        GP.TITLE("MULTI-VEND");
        GP.LABEL("Автомойка самообслуживания");
        ...
        GP.BLOCK_THIN_TAB_BEGIN(F("Ручное обновление"));

        if (!UpdateEnd) {
            GP.OTA_FIRMWARE(F("Выбрать файл"), GP_GREEN, true /*OTA page*/);
            // GP.OTA_FILESYSTEM(F("OTA filesystem"), GP_GREEN, false /*OTA page*/);
        } else if (UpdateError.length()) {
            GP.LABEL(String(F("Ошибка обновления")));
            //GP.BUTTON_LINK(F("/ota_update"), F("Refresh"));
        } else {
            GP.LABEL(F("Обновление успешно!"));
            GP.LABEL(F("Перезагрузка..."));

            //GP.BUTTON_LINK(F("/ota_update"), F("Refresh"));
        }
        
        GP.BLOCK_END();
        GP.BUTTON_LINK(F("/"), F("Вернуться в меню"));

        GP.BREAK();
        GP.HR();
        GP.BREAK();

        GP.LABEL("Copyright © multi-vend.ru");
        GP.BREAK();
        GP.BREAK();
        GP.LABEL("2023");
